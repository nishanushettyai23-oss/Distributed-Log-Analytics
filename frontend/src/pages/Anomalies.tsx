...
import { api } from "../api";
import { EmptyState, ErrorState, Kpi, PageHeader, Panel, formatNumber } from "../components/UI";
  ...
  return <div>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-3">
      ...
      <Panel title="Detected Risk Events" className="xl:col-span-3">
        {items.length?<div className="max-h-96 overflow-auto scrollbar"><table className="w-full min-w-[800px] text-left text-sm"><thead className="sticky top-0 bg-white dark:bg-slate-900"><tr>{["Severity","Score","Component","Node","Hour","Error Code","Events"].map(h => <th key={h} className="border-b border-slate-200 px-3 py-2 dark:border-slate-700">{h}</th>)}</tr></thead><tbody>{items.map((item,i) => <tr key={i} className="border-b border-slate-100 dark:border-slate-800"><td className="px-3 py-2"><span className={`rounded px-2 py-1 text-xs font-semibold ${item.severity==="critical"?"bg-red-100 text-red-700":item.severity==="high"?"bg-amber-100 text-amber-700":"bg-blue-100 text-blue-700"}`}>{item.severity}</span></td><td className="px-3 py-2 font-mono">{item.anomaly_score}</td><td className="px-3 py-2">{item.component}</td><td className="px-3 py-2">{item.node_id}</td><td className="px-3 py-2">{item.hour}</td><td className="px-3 py-2">{item.error_code}</td><td className="px-3 py-2">{item.event_count}</td></tr>)}</tbody></table></div>:<EmptyState title="No statistical anomalies detected" detail="No component-hour group exceeded the configured z-score threshold of 2.0 in the processed dataset. Zero is a valid analysis result, not missing data."/>}
      </Panel>
    </div>
  </div>;
}
