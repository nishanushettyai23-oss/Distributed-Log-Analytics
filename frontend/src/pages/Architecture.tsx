...
const initialNodes: Node[] = [
  ...
  {id:"dataproc",position:{x:440,y:120},data:{label:"Dataproc Cluster\n1 master + workers"},sourcePosition:Position.Right,targetPosition:Position.Left},
  ...
  {id:"react",position:{x:1320,y:120},data:{label:"React Platform\nEnterprise observability"},sourcePosition:Position.Right,targetPosition:Position.Left}
];
...
export default function Architecture() {
  ...
  return <div><PageHeader title="Cloud Architecture" subtitle="Interactive end-to-end view of the distributed processing and visualization pipeline."/><div className="grid gap-5 xl:grid-cols-[1fr_320px]"><Panel className="h-[620px] overflow-hidden p-0"><ReactFlow nodes={initialNodes} edges={edges} onNodeClick={onNodeClick} fitView minZoom={0.4}><Background/><Controls/></ReactFlow></Panel><Panel title={initialNodes.find(n=>n.id===selected)?.data.label.split("\n")[0]}><p className="text-sm leading-6 text-slate-600 dark:text-slate-300">{details[selected]}</p><div className="mt-5 border-t border-slate-200 pt-4 text-xs text-slate-500 dark:border-slate-700">Click any architecture node to inspect its technology, role, and processing responsibility.</div></Panel></div></div>;
}
