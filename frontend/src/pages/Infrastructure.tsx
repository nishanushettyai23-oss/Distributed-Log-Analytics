...
export default function Infrastructure() {
  ...
  const buckets=query.data?.storage?.buckets||[];
  ...
  return <div><PageHeader title="Infrastructure" subtitle="Cloud operations view of Dataproc, Google Cloud Storage, BigQuery, and the latest distributed Spark execution."/>
    ...
    <div className="mt-5 grid gap-5 xl:grid-cols-2"><Panel title="Dataproc Cluster"><dl className="grid grid-cols-2 gap-4 text-sm">{Object.entries(d).map(([k,v])=><div key={k}><dt className="text-slate-500">{k.replace(/_/g," ")}</dt><dd className="mt-1 break-words font-semibold">{String(v)}</dd></div>)}</dl></Panel><Panel title="Cloud Storage">{buckets.length?<div className="space-y-4">{buckets.map(bucket=><div key={bucket.name} className="rounded-md border border-slate-200 p-3 text-sm dark:border-slate-700"><div className="break-all font-mono font-semibold">{bucket.name}</div><div className="mt-2 flex flex-wrap gap-4 text-xs text-slate-500"><span>{formatNumber(bucket.objects)} objects</span><span>{bucket.bytes ? `${(bucket.bytes/1073741824).toFixed(2)} GB` : "Size unavailable"}</span><span>{bucket.location || "Location unavailable"}</span></div></div>)}</div>:<p className="text-sm text-slate-500">Storage metadata is currently unavailable.</p>}</Panel>
      <Panel title={`BigQuery - ${b?.dataset||"logs_dataset"}`} className="xl:col-span-2"><div className="overflow-auto"><table className="w-full text-left text-sm"><thead><tr><th className="border-b py-2 dark:border-slate-700">Table</th><th className="border-b py-2 dark:border-slate-700">Rows</th><th className="border-b py-2 dark:border-slate-700">Storage</th></tr></thead><tbody>{tables.map(t=><tr key={t.name}><td className="border-b py-3 font-mono dark:border-slate-800">{t.name}</td><td className="border-b py-3 dark:border-slate-800">{formatNumber(t.rows)}</td><td className="border-b py-3 dark:border-slate-800">{(Number(t.bytes||0)/1073741824).toFixed(2)} GB</td></tr>)}</tbody></table>{!tables.length&&<p className="py-6 text-center text-sm text-slate-500">BigQuery table metadata is currently unavailable.</p>}</div></Panel></div>
  </div>;
}
